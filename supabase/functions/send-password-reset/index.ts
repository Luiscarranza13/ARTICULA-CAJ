...
Deno.serve(async (request) => {
  ...
  try {
    ...
    const message = error instanceof Error ? error.message : 'No se pudo enviar recuperacion';
...
async function generateRecoveryLink(email: string) {
  const supabaseUrl = requiredSecret('APP_SUPABASE_URL');
  const serviceRoleKey = requiredSecret('APP_SUPABASE_SERVICE_ROLE_KEY');
  ...
  if (!link) throw httpError(502, 'Supabase no devolvio enlace de recuperacion');
  return link;
}
...
function buildPasswordResetEmail(email: string, resetUrl: string) {
  ...
  const text = [
    'Recibimos una solicitud para cambiar tu contrasena en ARTICULA CAJ.',
    '',
    `Correo: ${email}`,
          ...
          <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="width:100%;max-width:680px;background:#ffffff;border:1px solid #dbe7e1;border-radius:24px;overflow:hidden;">
            <tr>
              <td style="background:#064e3b;background-image:linear-gradient(135deg,#052e24 0%,#047857 58%,#16a34a 100%);padding:30px;color:#ffffff;">
                ...
                <h1 style="margin:8px 0 10px;font-size:30px;line-height:1.16;font-weight:900;color:#ffffff;">Restablece tu contrasena</h1>
                <p style="margin:0;color:#ecfdf5;font-size:15px;line-height:1.65;">Recibimos una solicitud para cambiar la contrasena de tu cuenta.</p>
              </td>
            </tr>
            <tr>
              <td style="padding:30px;">
                <div style="background:#f8fafc;border:1px solid #e2e8f0;border-radius:18px;padding:18px 20px;color:#334155;font-size:15px;line-height:1.7;">
                  Usa el siguiente boton para crear una nueva contrasena para <strong style="color:#0f172a;">${safeEmail}</strong>. Por seguridad, este enlace es temporal.
                </div>
                ...
                <p style="margin:24px 0 0;color:#94a3b8;font-size:12px;line-height:1.6;">Si no solicitaste este cambio, puedes ignorar este mensaje. Tu cuenta seguira protegida.</p>
              </td>
            </tr>
          </table>
        </td>
